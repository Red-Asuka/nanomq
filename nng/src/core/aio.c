...
static void nni_aio_expire_rm(nni_aio_expire_q *, nni_aio *);
...
static void nni_aio_expire_rm(nni_aio_expire_q * eq, nni_aio * aio)
{
...
nni_aio_expire_add(nni_aio *aio)
{
	...
	*/
	...
	if (eq->eq_len > eq->eq_sz * 2 / 3) {
		...
		nni_free(eq->eq_list, eq->eq_sz);
		eq->eq_list = new_list;
		eq->eq_sz *= 2;
		fprintf(stderr, "expire_q [%d/%d]", eq->eq_len, eq->eq_sz);
	}
...
nni_aio_expire_loop(void *arg)
{
	...
	for (;;) {
		...
		for (uint32_t i=aio_index+1; i<q->eq_len; i++) {
			list[i-1] = list[i];
		}
		q->eq_len --;
...
nni_aio_expire_q_alloc(void)
{
	...
	eq->eq_sz = 1024;
	eq->eq_len = 0;
	eq->eq_list = nni_zalloc(eq->eq_sz * sizeof(nni_aio *));
	eq->eq_exit = false;
